...
`state.md`와 `output.md`는 최신 상태만 유지합니다. 과거 근거와 분석은 `research.md`·`reference/`에서 삭제하지 않고 대체 관계를 표시합니다.
...
`state.md`에는 최소한 persona ID, 상태, 현재 과업, 초점, 차단 요인, 다음 행동, 현재 정본, 마지막 갱신을 둡니다.

`output.md`에는 현재 결론, 가장 큰 한계, 다음 제안, 정본 링크가 드러나야 합니다. 질문에 따라 시장진입·예산·P0·파트너십 같은 역할별 필드를 추가할 수 있습니다. 알 수 없는 값은 삭제하거나 0으로 만들지 않고 `미측정` 또는 `입력 대기`로 둡니다.
...
- persona output은 다음 로컬의 짧은 역할 인계이며 모든 역할을 한 화면에 자동 복제하지 않습니다.
