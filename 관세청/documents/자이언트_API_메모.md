...
> 대조 기준: **공개 v2.0.0 (2026-08-20)** · 원본 대조일 2026-08-26 · 협의 델타 반영일 2026-09-01
> 규칙 — 자이언트 규격은 **Postman 화면으로 판단하지 않는다.** 컬렉션 원본 JSON을 내려받아 대조한다. 화면은 지연 로딩되어 일부만 보인다(2026-08-10 오판 원인).

---

## 0. 2026-09-01 운영전환 확정 — 동수 V2-1 + Geek V2-2 우선 병합

근거 원문은 `documents/20260901_자이언트_운영전환_협의원문.md`에 보존한다. 공개 컬렉션의 버전 설명이 아니라 **사용자 지시와 자이언트 담당자 운영 회신으로 확정한 현장 적용 기준**이다.

| 역할 | 전송·처리 기준 | 확정 범위 |
|---|---|---|
| 동수 | 전 주문을 **V2-1** 기준으로 자이언트에 전달한다. 대화 원문의 `v1 gate`·`v1`은 공유 문서에서 `V2-1`로 정규화한다. | 2026-09-01 사용자 확정 |
| Geek | 자사몰의 PER 인증 결과와 통관·수하인 데이터를 **V2-2 `clearance`**로 동일 `hwbNo + orderNo`에 전달한다. | 2026-09-01 사용자 확정 |
| 자이언트 | 같은 키에서 V2-1과 V2-2 필드가 겹치면 **Geek V2-2에서 받은 유효한 값을 우선 적용**한다. V2-2에 없는 물류·상품 필드는 동수 V2-1 값을 유지한다. | 사용자 지시 + 10:54 자이언트 회신 |

우선순위 로직은 다음처럼 한정한다.

1. 병합 키는 기존과 같이 `hwbNo + orderNo`다.
2. V2-2가 실제로 보낸 **비어 있지 않고 스키마 검증을 통과한 필드**만 동일 필드의 V2-1 값을 덮어쓴다.
3. V2-2의 `null`·빈 문자열은 V2-1의 정상값을 지우지 않는다. 명시적 삭제 기능이 확인되기 전에는 빈 값 덮어쓰기를 금지한다.
4. Geek 우선 범위는 통관·인증 스냅샷의 겹치는 필드다. 동수 소유 물류·상품 필드는 V2-2가 덮어쓰지 않는다.
5. 최종 레코드에는 필드별 `appliedSource`(`DONGSU_V2_1` 또는 `GEEK_V2_2`), 수신시각, 원본 값의 해시 또는 마스킹 로그, 덮어쓰기 결과를 남겨야 한다. 현재 이 조회 수단은 확인되지 않았다.

### 2026-09-01 운영 사고와 전환 상태

- 2026-08-31 문제 주문은 **90건**으로 언급됐다. 구 API로 전달되어 신규 신고용 업체부호·일회용 인증번호 등이 자이언트에 전달되지 않았다.
- 해당 90건을 신규 신고로 처리하려면 2026-09-01 **13:00~14:00 전까지** 신규 신고 필수데이터 전부를 줘야 한다는 조건만 확인됐다. 전량 보완·제출·접수 완료는 미확인이다.
- 일부 주문은 데이터 수집 오류로 개인통관고유부호가 미확보 상태다. 개발로그에서 회수한 뒤 수기로 전달하기로 했으나 확보 건수·전달 완료시각은 미확인이다.
- `O000000000004`는 형식 검사를 통과할 수 있으나 자이언트가 임의값 가능성을 제기했다. 진위 검증이 되지 않았으므로 가짜로 확정하지 않고 격리한다.
- 동수는 2026-09-01 개발 완료, 2026-09-02 주문부터 V2-1 적용 예정으로 정리됐다. 실제 운영 배포와 첫 성공 응답은 별도 확인한다.
- 전환기에는 기존 신고와 전자상거래 전용 신고가 혼재한다. 대화의 “어제부터 향후 3일”은 종료 일시가 모호하므로 주문별 신고방식과 전환시각을 WeChat에 기록하고 정확한 종료시각을 다시 확정한다.
- 자이언트는 V2-1에 일회용 번호가 없을 수 있음을 인지하고 “수하인정보 검증하고 개인통관고유부호는 제외”한다고 회신했다. 이를 **자이언트의 단기 입력 검증 예외**로만 해석하며, 세관 신고에서 PCCC가 불필요하다는 뜻으로 확대하지 않는다.

### 2026-09-01 남은 확인사항

1. Geek 우선이 적용되는 정확한 필드 목록과 `null`·빈 문자열·형식 오류일 때의 처리.
2. V2-1 선도착·V2-2 후도착, 반대 순서, 병합 완료 후 V2-2 재전송의 동작과 `H019` 적용 범위.
3. 최종 적용값과 원천을 키별·필드별로 조회할 API 또는 원본 로그.
4. 개인통관고유부호를 “제외”한다는 검증 예외의 정확한 범위·종료일·신고 단계 보완 책임.
5. 90건별 필수데이터 보완 여부, 최종 신고방식, 접수번호·오류코드·신고 완료 여부.

---

## 0-A. 2026-08-28 최종 정정 — A/B와 신고서식을 분리한다
...
## 0-B. 2026-08-28 B유형 조건부 필드 협의 — 후속 정정 전 협의 이력
